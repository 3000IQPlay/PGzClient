...
class EntityNotifier
    ...
    private final TrayIcon icon = new TrayIcon ( this.image , "PGzClient" );
    ...
    void onUpdate ( ) {
        if ( this.Ghasts.getValue ( ) ) {
            for (Entity entity : EntityNotifier.mc.world.getLoadedEntityList ( )) {
                ...
                this.icon.displayMessage ( "PGzClient" , "I found a ghast at: " + Math.round ( entity.getPosition ( ).getX ( ) ) + "x, " + Math.round ( entity.getPosition ( ).getY ( ) ) + "y, " + Math.round ( entity.getPosition ( ).getZ ( ) ) + "z." , TrayIcon.MessageType.INFO );
        ...
        if ( this.Donkeys.getValue ( ) ) {
            for (Entity entity : EntityNotifier.mc.world.getLoadedEntityList ( )) {
                ...
                if ( ! this.Desktop.getValue ( ) ) continue;
                this.icon.displayMessage ( "PGzClient" , "I found a donkey at: " + Math.round ( entity.getPosition ( ).getX ( ) ) + "x, " + Math.round ( entity.getPosition ( ).getY ( ) ) + "y, " + Math.round ( entity.getPosition ( ).getZ ( ) ) + "z." , TrayIcon.MessageType.INFO );
                if ( ! this.Sound.getValue ( ) ) continue;
                EntityNotifier.mc.player.playSound ( SoundEvents.BLOCK_ANVIL_DESTROY , 1.0f , 1.0f );
            }
        }
        if ( this.Mules.getValue ( ) ) {
            for (Entity entity : EntityNotifier.mc.world.getLoadedEntityList ( )) {
                ...
                if ( ! this.Desktop.getValue ( ) ) continue;
                this.icon.displayMessage ( "PGzClient" , "I found a mule at: " + Math.round ( entity.getPosition ( ).getX ( ) ) + "x, " + Math.round ( entity.getPosition ( ).getY ( ) ) + "y, " + Math.round ( entity.getPosition ( ).getZ ( ) ) + "z." , TrayIcon.MessageType.INFO );
                if ( ! this.Sound.getValue ( ) ) continue;
                EntityNotifier.mc.player.playSound ( SoundEvents.BLOCK_ANVIL_DESTROY , 1.0f , 1.0f );
            }
        }
        if ( this.Llamas.getValue ( ) ) {
            for (Entity entity : EntityNotifier.mc.world.getLoadedEntityList ( )) {
                ...
                if ( ! this.Desktop.getValue ( ) ) continue;
                this.icon.displayMessage ( "PGzClient" , "I found a llama at: " + Math.round ( entity.getPosition ( ).getX ( ) ) + "x, " + Math.round ( entity.getPosition ( ).getY ( ) ) + "y, " + Math.round ( entity.getPosition ( ).getZ ( ) ) + "z." , TrayIcon.MessageType.INFO );
                if ( ! this.Sound.getValue ( ) ) continue;
                EntityNotifier.mc.player.playSound ( SoundEvents.BLOCK_ANVIL_DESTROY , 1.0f , 1.0f );
            }
        }
