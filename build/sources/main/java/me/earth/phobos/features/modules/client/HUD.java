...
class HUD
    ...
    private static final ResourceLocation codHitmarker = new ResourceLocation ( "PGzClient" , "cod_hitmarker" );
    public static final SoundEvent COD_EVENT = new SoundEvent ( codHitmarker );
    private static final ResourceLocation csgoHitmarker = new ResourceLocation ( "PGzClient" , "csgo_hitmarker" );
    ...
    private final Setting < String > customWatermark = this.register ( new Setting <> ( "WatermarkName" , "PGzClient v2" ) );
    private final Setting < Boolean > modeVer = this.register ( new Setting < Object > ( "Version" , Boolean.TRUE , v -> this.watermark.getValue ( ) != WaterMark.NONE ) );
    ...
    private final Setting < Boolean > ping = this.register ( new Setting <> ( "Ping" , Boolean.TRUE , "Your response time to the server." ) );
    private final Setting < Boolean > tps = this.register ( new Setting <> ( "TPS" , Boolean.TRUE , "Ticks per second of the server." ) );
    private final Setting < Boolean > fps = this.register ( new Setting <> ( "FPS" , Boolean.TRUE , "Your frames per second." ) );
    private final Setting < Boolean > coords = this.register ( new Setting <> ( "Coords" , Boolean.TRUE , "Your current coordinates" ) );
    private final Setting < Boolean > direction = this.register ( new Setting <> ( "Direction" , Boolean.TRUE , "The Direction you are facing." ) );
    ...
    private final Setting < Boolean > armor = this.register ( new Setting <> ( "Armor" , Boolean.TRUE , "ArmorHUD" ) );
    private final Setting < Boolean > durability = this.register ( new Setting <> ( "Durability" , Boolean.TRUE , "Durability" ) );
    ...
    private final Setting < Boolean > queue = this.register ( new Setting <> ( "2b2tQueue" , Boolean.FALSE , "Shows the 2b2t queue." ) );
    private final Setting < Greeter > greeter = this.register ( new Setting <> ( "Greeter" , Greeter.NONE , "Greets you." ) );
    private final Setting < String > spoofGreeter = this.register ( new Setting < Object > ( "GreeterName" , "PGzClient" , v -> this.greeter.getValue ( ) == Greeter.CUSTOM ) );
    private final Setting < LagNotify > lag = this.register ( new Setting <> ( "Lag" , LagNotify.RED , "Lag Notifier" ) );
    ...
    public Setting < Integer > hudRed = this.register ( new Setting < Object > ( "Red" , 40 , 0 , 255 , v -> ! this.rainbow.getValue ( ) ) );
    public Setting < Integer > hudGreen = this.register ( new Setting < Object > ( "Green" , 160 , 0 , 255 , v -> ! this.rainbow.getValue ( ) ) );
    ...
    @SubscribeEvent
    public
    void onModuleToggle ( final ClientEvent event ) {
    ...
    void onRender2D ( final Render2DEvent event ) {
        ...
        switch (this.watermark.getValue ( )) {
            case PGZ: {
                this.renderer.drawString ( "PGzClient" + ( this.modeVer.getValue ( ) ? " v2" : "" ) , 2.0f , 2.0f , ( this.rolling.getValue ( ) && this.rainbow.getValue ( ) ) ? this.colorMap.get ( 2 ) : this.color , true );
                break;
            }
            case DUCK: {
                this.renderer.drawString ( "DuckClient" + ( this.modeVer.getValue ( ) ? " v2" : "" ) , 2.0f , 2.0f , ( this.rolling.getValue ( ) && this.rainbow.getValue ( ) ) ? this.colorMap.get ( 2 ) : this.color , true );
                break;
            }
            case CUSTOM: {
                this.renderer.drawString ( this.customWatermark.getValue ( ) + ( this.modeVer.getValue ( ) ? " v2" : "" ) , 2.0f , 2.0f , ( this.rolling.getValue ( ) && this.rainbow.getValue ( ) ) ? this.colorMap.get ( 2 ) : this.color , true );
                break;
            }
        }
        ...
        if ( this.arrayList.getValue ( ) ) {
            if ( this.renderingUp.getValue ( ) ) {
                ...
                for (int k = 0; k < ( this.alphabeticalSorting.getValue ( ) ? Phobos.moduleManager.alphabeticallySortedModules.size ( ) : Phobos.moduleManager.sortedModules.size ( ) ); ++ k) {
                    ...
                    j += 10;
                    renderer.drawString ( text , x , (float) ( height - j ) , ( this.rolling.getValue ( ) && this.rainbow.getValue ( ) ) ? this.colorMap.get ( MathUtil.clamp ( height - j , 0 , height ) ) : ( ( this.moduleColors.getValue ( ) && moduleColor != null ) ? moduleColor.getRGB ( ) : this.color ) , true );
                }
            }
        }
    ...
    void renderGreeter ( ) {
        ...
        switch (this.greeter.getValue ( )) {
            ...
            case LONG: {
                text = text + "Welcome to PGzClient " + HUD.mc.player.getDisplayNameString ( ) + " :^)";
                break;
            }
            case CUSTOM: {
    ...
    public
    void renderArmorHUD ( final boolean percent ) {
        final int width = this.renderer.scaledWidth;
        final int height = this.renderer.scaledHeight;
        GlStateManager.enableTexture2D ( );
        final int i = width / 2;
    ...
    public
    void drawTextRadar ( final int yOffset ) {
